...
function Product({ data, onEdit }: { data: ProductData | null; onEdit: () => void }) {
  ...
  return <>
    ...
    <section className="product-hero-grid">
      <article className="nps-card"><div><span>NPS · ТЕКУЩИЙ ПЕРИОД</span><strong>{metrics.nps}</strong><Trend value="6 пунктов" /></div><div className="nps-big-ring"><span><b>81%</b><small>промоутеры</small></span></div><p>Обновлено из продуктового контура</p></article>
      <article className="panel students-card"><div className="panel-head"><div><span className="panel-kicker">УЧЕНИКИ</span><h3>{metrics.activeStudents} активных</h3></div><Trend value="12,4%" /></div><div className="cohort-bars">{[52,68,73,82,88,75,91,96].map((v,i)=><i key={i} style={{height:`${v}%`}}><span>{v}</span></i>)}</div><div className="cohort-labels"><span>Нед. 1</span><span>Нед. 8</span></div></article>
