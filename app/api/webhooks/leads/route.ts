...
import { activityEvents, clients, managers } from "../../../../db/schema";
...
export async function POST(request: Request) {
  try {
    ...
    if (!expected) return Response.json({ error: "Webhook не активирован: добавьте LEAD_WEBHOOK_SECRET" }, { status: 503 });
    if (request.headers.get("x-webhook-secret") !== expected) return Response.json({ error: "Unauthorized" }, { status: 401 });
    ...
    const db = getDb();
    let manager = String(payload.manager || "").trim();
    if (!manager) {
      const [managerRows, clientRows] = await Promise.all([db.select().from(managers), db.select().from(clients)]);
      const active = managerRows.filter(item => item.status === "active");
      if (active.length) {
        const selected = active.sort((a, b) => {
          const aName = a.name.split(" ")[0];
          const bName = b.name.split(" ")[0];
          return clientRows.filter(item => item.manager === aName).length - clientRows.filter(item => item.manager === bName).length;
        })[0];
